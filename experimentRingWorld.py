...
from Agents.RingWorldAgents.agentRNNTDZeroOffpolicy import RNNAgent
...
ringworld_size = 6
gamma = 1.0
truncation_length = [1,2,4] #1,2,4,6,10,16
learning_rate = 0.0005
hiddenstate_size = 6 #3,6,9,12


for j in range(len(truncation_length)):
	for run in runs:

		end = False
		environment = RingWorld(ringworld_size = ringworld_size)
		
		agent = RNNAgent(gamma = gamma, truncation_length = truncation_length[j],
