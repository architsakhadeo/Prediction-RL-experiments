...
class RNNState(nn.Module):
	...
	def forward(self, old_observation, action):
		...
		self.prediction = self.fc(self.output)
...
class RNNAgent():
	...
	def start(self, observation, run):
		...
		self.finalrewardslist = np.array([])
	...
	def step(self, observation, term, end, run):
		'''
		...
		if end == True:
			pickle.dump(self.finallosslist, open('Data/CompassWorld/run'+str(run)+'_loss','wb'))
			#pickle.dump(self.finalrewardslist, open('Data/CompassWorld/run'+str(run)+'_rewards','wb'))
	...
	def train(self, old_observation, new_observation, action, term):
		...
		self.value_old_state_action = self.model.forward(old_observation, action)
		value_new_state_L = self.model.forward(new_observation, torch.tensor([1, 0, 0], dtype=torch.float))
		value_new_state_F = self.model.forward(new_observation, torch.tensor([0, 1, 0], dtype=torch.float))
		value_new_state_R = self.model.forward(new_observation, torch.tensor([0, 0, 1], dtype=torch.float))
		'''
		...
		self.target_value_new_state = (value_new_state_L + value_new_state_F + value_new_state_R)/3.0
		...
		#self.finalrewardslist = np.append(self.finalrewardslist, reward)
	...
	def backward(self, new_observation, prediction):
		...
		delta = new_observation + prediction
		self.loss = torch.mean( (delta) ** 2 )
		loss = self.loss.item()
